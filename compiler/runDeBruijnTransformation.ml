...
(**
	...
	is parsed to obtain the AST. The De	Bruijn index transformation will
	then be applied to the AST. Afterwards the transformed version will
	be printed.
*)
let main () = (* Parsing arguments *)
  ...
  in
    ...
    (Arg.parse [] (fun s -> f_name := s) "ML compiler";
     ...
     in
				print_string "De Bruijn transformation...\n";
        let deBruijn_prog = Elaboration.deBruijnTransform ml_prog
        in
